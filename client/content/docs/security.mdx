...
Backups are created with SQLite's `VACUUM INTO` and written as encrypted `.acxb` files — they carry no plaintext, so a backup on an external drive or in the cloud stays as protected as your live book. See [Backup & restore](/docs/backup-and-restore/).
